...
void solve()
{
    ...
    for (int k = 0; k < TRIALS; ++k)
    {
        ...
        if (!succ) std::swap(perm[i], perm[j]);

        if (k % (TRIALS / 100) == 0) std::cerr << "Curr LO score: " << bestScore << " " << (k * 100 + TRIALS / 2) / TRIALS << "%" << std::endl;
    }
...
int main()
{
    ...
    outFile.open(("lo_" + tests[test] + ".txt").c_str());
    bestFile.open(("gr_" + tests[test] + ".txt").c_str());
